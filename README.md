...
- **Supabase Edge Functions** (Deno runtime) for all server-side LLM calls
- **OpenAI `gpt-4o-mini`** for the AI Strategy Assistant's priority insight, the action page's evidence advice, and the Case Chat Assistant replies

No global state library. No backend besides three Supabase Edge Functions (`priority-insight`, `evidence-advice`, `case-chat`). Fixtures load synchronously at import time.
...
1. In your Supabase project (or Lovable's Cloud tab), set a secret named `OPENAI_API_KEY` (no `VITE_` prefix — that would inline it into the browser bundle, defeating the security boundary). The same secret is reused by all three edge functions.
2. Deploy the edge functions: `supabase functions deploy priority-insight evidence-advice case-chat` (Lovable does this automatically on commit).
...
- Case Chat — `buildCaseContext` shape + `sendCaseChatMessage` success, error, timeout, and anon-key fallback paths
...
│   │   ├── ai.ts                         Triage counts, priority selection, priority-insight + evidence-advice edge calls
│   │   ├── caseChat.ts                   buildCaseContext + sendCaseChatMessage (case-chat edge call)
...
├── supabase/
│   └── functions/
│       ├── priority-insight/index.ts     Edge function — OpenAI proxy for morning-triage sentence
│       ├── evidence-advice/index.ts      Edge function — OpenAI proxy for action-page evidence advice
│       └── case-chat/index.ts            Edge function — OpenAI proxy for per-case chat replies
├── docs/
...
- LLM usage is scoped: one call per render of `/` (priority insight), one per render of an action page (evidence advice), and one per user turn in the case chat panel. All three are edge-function-proxied with deterministic fallbacks.
