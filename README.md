...
| `/case/:caseId` | Case detail — applicant header with risk badge, timeline, policy accordion, evidence tracker (received / outstanding / overdue), workflow state + required next actions, live GOV.UK guidance from the Content API. |
| `/case/:caseId/action/:actionId` | Action stub — read-only "mock action page" listing the GOV.UK pages a caseworker would need to navigate to perform that action. No state mutation. |
...
- **Supabase Edge Functions** (Deno runtime) for the one server-side LLM call
- **OpenAI `gpt-4o-mini`** for the AI Strategy Assistant's priority-insight sentence

No global state library. No backend besides the single Supabase Edge Function. Fixtures load synchronously at import time.
...
The AI Strategy Assistant works without configuration — it falls back to a deterministic priority-insight sentence. To enable the LLM-paraphrased sentence:
...
These are the only client-side values. The OpenAI API key lives **server-side** as a Supabase secret named `OPENAI_API_KEY` on the `priority-insight` Edge Function — see [AI Strategy Assistant — setup](#ai-strategy-assistant--setup) below.

## AI Strategy Assistant — setup

The sidebar's priority-insight sentence is generated by `gpt-4o-mini` via a Supabase Edge Function (`supabase/functions/priority-insight/index.ts`). The OpenAI key never enters the browser bundle — the frontend talks only to the edge function, and the function holds the key.

### Deployment (Supabase / Lovable)

1. In your Supabase project (or Lovable's Cloud tab), set a secret named `OPENAI_API_KEY` (no `VITE_` prefix — that would inline it into the browser bundle, defeating the security boundary).
2. Deploy the edge function: `supabase functions deploy priority-insight` (Lovable does this automatically on commit).
3. Set `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` in `.env.local` for local dev, or in your hosting provider's environment for production.
4. Set a usage cap on the OpenAI account dashboard.
...
2. It POSTs the chosen case's reference, applicant, risk factors, applicable policy, and recommended action label to `/functions/v1/priority-insight`.
3. The edge function calls OpenAI with `temperature: 0.2`, `max_tokens: 180`, and a system prompt that forbids fabricating identifiers.
4. The frontend validates that the returned text contains the case reference, action label, and (when supplied) policy ID. If validation fails — or if the call times out (>5s) or returns non-2xx — the panel renders a deterministic fallback sentence composed from the same inputs.
...
Tests cover the derivations most likely to silently drift:
...
- AI assistant — deterministic case selection across filter combinations
- AI assistant — triage counts match the visible caseload
- AI assistant — deterministic fallback when the network is stubbed to fail

UI correctness is verified by visual inspection rather than component tests, by design.
...
│   ├── components/
│   │   ├── ai/                           AI Strategy Assistant + WorkloadHeatmap
...
│   │   └── RiskBadge.tsx                 Coloured risk pill with HoverCard
│   ├── lib/
...
│   │   ├── CaseDetail.tsx                /case/:caseId
│   │   ├── ActionStub.tsx                /case/:caseId/action/:actionId
│   │   └── NotFound.tsx                  404
│   ├── services/
│   │   ├── cases.ts                      Service surface over fixtures + enrichment
│   │   ├── ai.ts                         Triage counts, priority selection, edge-function call
│   │   └── govuk.ts                      Live fetch from gov.uk/api/search.json
│   ├── test/
│   │   ├── services.test.ts              Stream A derivation tests
│   │   └── assistant.test.ts             Stream F selection + fallback tests
│   └── types/case.ts                     Frozen type contracts
├── supabase/
│   └── functions/
│       └── priority-insight/index.ts     Edge function — OpenAI proxy
├── docs/
...
The app exposes three React Router routes only. There is no write endpoint, no persistence, no toast that implies state mutation. The AI Strategy Assistant and WorkloadHeatmap are panels embedded on `/`, not new routes.
...
- No write operations. The action stub is a placeholder; the AI assistant call is read-only.
- Exactly one scoped LLM call per render of `/`. Other pages are LLM-free.
